// ============================================================
//  STOCKPILOT — Main Application Logic (Firebase Compat)
// ============================================================
...
$("login-btn").addEventListener("click", async () => {
  ...
  hide(errEl);
  if (!email || !pass) {
    show(errEl); errEl.textContent = "Please enter your email and password."; return;
  }
...
function subscribeInventory() {
  ...
  inventoryUnsub = db.collection("inventory")
...
function renderDashboard() {
  ...
  const val = allItems.reduce((s, i) => s + ((i.unitPrice || 0) * (i.quantity || 0)), 0);
...
function renderInventory(filter, cat, sort) {
  filter = filter ?? $("search-box").value;
  cat    = cat    ?? $("cat-filter").value;
  sort   = sort   ?? $("sort-select").value;
  const isAdmin = currentRole === "admin";
  let items = [...allItems];
  if (filter) items = items.filter(i => (i.name + i.sku + i.category).toLowerCase().includes(filter.toLowerCase()));
  if (cat)    items = items.filter(i => i.category === cat);
  if (sort === "low-stock")  items.sort((a, b) => getStockPriority(a) - getStockPriority(b));
  else if (sort === "qty-asc")   items.sort((a, b) => (a.quantity ?? 0) - (b.quantity ?? 0));
  else if (sort === "qty-desc")  items.sort((a, b) => (b.quantity ?? 0) - (a.quantity ?? 0));
  ...
  $("inventory-body").innerHTML = items.length
    ? items.map(item => `<tr>
        ...
        <td>${statusBadge(item)}</td>
        ${isAdmin
          ? `<td class="admin-only"><div class="action-btns">
...
$("search-box").addEventListener("input",   () => renderInventory());
...
$("save-item-btn").addEventListener("click", async () => {
  ...
  try {
    if (editId) {
      await db.collection("inventory").doc(editId).update(payload);
      toast("Item updated.");
    } else {
...
window.confirmDelete = function(type, id) {
  ...
  $("confirm-msg").textContent = type === "user"
    ? "Delete this user profile? This only removes their profile record."
...
$("confirm-delete-btn").addEventListener("click", async () => {
  try {
    if (pendingDelete.type === "item") {
      await db.collection("inventory").doc(pendingDelete.id).delete();
      toast("Item deleted.");
    ...
    ? allUsers.map(u => `<tr>
        ...
        <td><div class="action-btns">
          ...
          ${u.id !== currentUser.uid
            ? `<button class="icon-btn del" onclick="confirmDelete('user','${u.id}')">✕ Del</button>`
...
window.openEditUser = function(uid) {
  ...
  $("create-user-fields").style.display = "none";
  hide($("user-modal-error"));
  openModal("user-modal");
};
...
$("save-user-btn").addEventListener("click", async () => {
  ...
  if (editingUserId) {
    try {
      ...
      loadUsers();
    } catch (e) { show(errEl); errEl.textContent = e.message; }
    return;
  }
  ...
  try {
    ...
    toast(`User ${name} created! You may need to sign back in.`);
    closeModal("user-modal");
    loadUsers();
    setTimeout(() => {
      if (auth.currentUser?.email !== currentUser.email) auth.signOut();
    }, 1000);
  } catch (e) {
    show(errEl);
...
function renderCSVPreview(rows) {
  ...
  $("csv-body").innerHTML = rows.slice(0, 20).map(r => `<tr>
    <td>${esc(r.name)}</td><td>${esc(r.sku||"—")}</td><td>${esc(r.category||"—")}</td>
    <td>${r.quantity||0}</td><td>₱${parseFloat(r.unit_price||0).toFixed(2)}</td>
  </tr>`).join("") + (rows.length > 20 ? `<tr><td colspan="5" class="empty-row">...and ${rows.length-20} more</td></tr>` : "");
}

$("csv-cancel").addEventListener("click", () => {
  csvRows = []; csvInput.value = "";
  show(importZone); hide($("csv-preview"));
});
  ...
  try {
    for (const r of csvRows) {
      await db.collection("inventory").add({
        ...
        quantity: parseInt(r.quantity)||0, unitPrice: parseFloat(r.unit_price)||0,
        threshold: parseInt(r.threshold)||5, description: r.description||"",
        createdAt: firebase.firestore.FieldValue.serverTimestamp(),
        updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
      count++;
...
function closeModal(id) { hide($(id)); hide($("modal-overlay")); }
